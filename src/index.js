...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (!user.pro && user.todos.length >= 10) {
    return response.status(403).json({ error: 'Free plan, already have 10 todos registered' });
  }
...
function checksTodoExists(request, response, next) {
  ...
  if (!validateId) {
    return response.status(400).json({ error: 'todo.id is not in the correct format' });
  }
